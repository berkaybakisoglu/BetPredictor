...
class DataLoader:
    ...
    def _split_data(self, data, test_mode):
        if test_mode:
            ...
            logger.info(f"Using seasons {test_seasons} for testing")
            
            test_data = data[data['Season'].isin(test_seasons)]
            train_data = data[~data['Season'].isin(test_seasons)]
        else:
            # For production mode, use the last 3 months of each season for testing
            train_data = []
            test_data = []
            
            for season in data['Season'].unique():
                season_data = data[data['Season'] == season]
                season_end_date = season_data['Date'].max()
                cutoff_date = season_end_date - pd.DateOffset(months=3)
                
                train_data.append(season_data[season_data['Date'] <= cutoff_date])
                test_data.append(season_data[season_data['Date'] > cutoff_date])
